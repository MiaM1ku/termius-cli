...
from .client.cryptor import RNCryptor, UnifiedCryptor
from .client.keyring import load_keyring
...
class CloudSynchronizationCommand(AbstractCommand):
    ...
    def take_action(self, parsed_args):
        ...
        pkset = {
            'public_key': self.config.get_safe('User', 'public_key'),
            'encrypted_private_key': self.config.get_safe(
                'User', 'encrypted_private_key'
            ),
            'encrypted_personal_key': self.config.get_safe(
                'User', 'encrypted_personal_key'
            ),
        }
        cryptor.keyring = load_keyring(
            controller.api, self.config, cryptor, pkset
        )

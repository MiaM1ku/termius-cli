...
class CryptoBulkEntryTransformer(BulkEntryTransformer):
    ...
    def to_model(self, payload):
        ...
        payload = self.crypto_controller.decrypt_payload(
            payload, crypto_fields=getattr(self.model_class, 'crypto_fields', None)
        )
        if isinstance(payload, dict) and payload.get('_undecryptable'):
            raise SkipField
